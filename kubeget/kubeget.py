...
# KUBEGET v.0.9. This program list the pods on the K8s cluster. 
# Commandline options:
...
def showPods(namespace, nodename):
    try:
        ...
        print('ERROR: Cant get the pods from the cluster')
    ...
    for pod in pods:
        ...
        if namespace == None:                                                       # if namespace not defined
            if nodename == None:                                                    # if namespace AND node not defined
                list_pod = True
            else:                                                                   # if namespace defined AND node not defined
                if nodename == pod['nodename']:
                    list_pod = True
        else:                                                                       # if namespace is defined
            if nodename == None:                                                    # if namespace is defined AND node not defined
                if namespace == pod['namespace']:
                    list_pod = True
            else:                                                                   # namespace is defined AND nodename also defined
                if (namespace == pod['namespace'] and nodename == pod['nodename']):
                    list_pod = True
        if list_pod:
...
if __name__ == '__main__':
    ...
    print("KUBEGET v.0.9. This program list the pods on the K8s cluster. Run with -h option for help.")
